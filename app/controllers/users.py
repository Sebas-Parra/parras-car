...
def get_user(user_id: UUID, db: Session = Depends(get_db), _=Depends(require_self_or_admin_user)):
    return user_service.get_user(db, user_id)


@router.put("/{user_id}", response_model=UserRead)
def update_user(user_id: UUID, data: UserUpdate, db: Session = Depends(get_db), _=Depends(require_self_or_admin_user)):
