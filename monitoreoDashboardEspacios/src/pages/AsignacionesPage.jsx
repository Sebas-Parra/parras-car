...
const AsignacionesPage = () => {
  ...
  const cargarFlota = useCallback(
    (uid) => {
      ...
      fetchFlota(uid, token)
        ...
        .catch((err) => {
          setFlota({ vehicles: [] });
          if (err.message && !err.message.includes('404')) toast.error(err.message);
        });
    },
    [token, toast],
  );
  ...
  useEffect(() => {
    fetchVehiculos(token).then((res) => setVehiculos(res.data)).catch((err) => {
      ...
      toast.error(err.message || 'No se pudieron cargar los vehículos');
    });
    if (canManage) {
      fetchUsers(token)
  ...
  useEffect(() => {
    ...
    fetchAsignacionesAudit(token, auditoriaPage, auditoriaPageSize)
      ...
      .catch((err) => {
        ...
        toast.error(err.message || 'No se pudo cargar la auditoría de asignaciones');
      });
  }, [token, canManage, auditoriaPage, auditoriaPageSize, toast]);
  ...
  const handleAsignar = async (e) => {
    ...
    try {
      ...
      toast.error(err.message || 'No se pudo asignar el vehículo');
    }
  };
  ...
  const handleQuitar = async (vehicleId) => {
    ...
    try {
      ...
      toast.error(err.message || 'No se pudo quitar la asignación');
    }
  };
  ...
  const handleTransferir = async (e) => {
    ...
    try {
      ...
      toast.success('Vehículo transferido correctamente.');
      setTransferVehiculo('');
      setTransferDesde('');
      setTransferHacia('');
      cargarFlota(usuarioObjetivo);
    } catch (err) {
      toast.error(err.message || 'No se pudo transferir el vehículo');
    }
  };
                ...
                <tbody className="divide-y divide-slate-100">
                  {flota.vehicles?.map((v) => (
                    <tr key={v.id}>
                      ...
                      <td className="px-2 py-2 text-sm text-slate-600">{toEnumLabel(v.tipo, TIPO_VEHICULO_LABELS)}</td>
